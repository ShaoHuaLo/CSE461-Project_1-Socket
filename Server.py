...
def response_to_client_a(addr_client):
    num, len0, udp_port, secretA = random.randint(0, 10), random.randint(0, 10), \
                                   random.randint(30000, 65535), random.randint(0, 50)
...
def response_to_client_b(socket_b, addr_client, psecret):
    tcp_port, secretB = random.randint(30000, 65535), random.randint(0, 1000)
...
def connect_to_TCP(tcp_port):
    ...
    # tcp_master_socket.settimeout(3)
...
def threaded(packet_recv, addr_client, sock):
    ...
    if not is_valid_packet_size(payload) or not is_valid_packet_a(header, payload):
        # print("row140: format of data is not correct") # for debugging
        return
    ...
    try:
        ...
        # one more check
        if count != num2:
            print("something weng wrong, packet might be missing")
            conn.close()
            return
